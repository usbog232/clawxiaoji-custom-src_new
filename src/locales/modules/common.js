...
export default {
  ...
  backendDownDesc: _('小技面板后端服务未运行，无法获取真实数据。', '小技面板后端服务未运行, so live data is unavailable.', 'ClawPanel 後端服務未執行，無法取得真實資料。', 'ClawPanel バックエンドが実行されていないため、実データを取得できません。', 'ClawPanel 백엔드가 실행 중이 아니어서 실제 데이터를 가져올 수 없습니다.', 'Dịch vụ backend của ClawPanel chưa chạy nên không thể lấy dữ liệu thực.'),
  ...
  pageLoadFailedHint: _('如果问题持续出现，请尝试重新安装小技面板，或反馈给项目维护者。', 'If the problem persists, try reinstalling Xiaoji Panel or report it to the project maintainer.', '如果問題持續出現，請嘗試重新安裝 ClawPanel，或到 GitHub Issues 回報。', '問題が続く場合は ClawPanel を再インストールするか、GitHub Issues で報告してください。', '문제가 계속되면 ClawPanel을 다시 설치하거나 GitHub Issues에 제보하세요.', 'Nếu lỗi vẫn lặp lại, hãy thử cài lại ClawPanel hoặc báo trên GitHub Issues.'),
